...
//
// Plus one project-specific gate: `forbiddenText`. Solange in src/data/site.ts
// noch Platzhalter stehen ("TODO — Anschrift" usw.), landen sie im HTML. Ohne
// --strict ist das eine Warnung, damit die Preview gebaut und angesehen werden
// kann. MIT --strict ist es ein harter Fehler — so kann die Seite nicht mit
// unvollständigem Impressum in Production gehen.
...
const DEFAULTS = {
  ...
  /** Zeichenketten, die im fertigen HTML nicht vorkommen dürfen. */
  forbiddenText: [],
};
...
for (const file of htmlFiles) {
  ...
  // verbotene Zeichenketten (unausgefüllte Platzhalter)
  for (const needle of cfg.forbiddenText) {
    if (!html.includes(needle)) continue;
    const msg = `Platzhalter im Build → "${needle}"`;
    if (strict) hard.push(msg);
    else warn.push(msg);
  }
